...
def get_articles_img_only_XAN(URL, filename):

    column = []
    temp=0
    try:
        df = pd.read_csv("static/data/Non_Detail/AnhNong/AnhNong.csv")
                ...
                if imgs.find('img'):
                    ...
                    for img in imgs1:
                        ...
                        if img.get('src'):
                            ...
                            file = open("static/data/Non_Detail/AnhNong/" + file_name + "_{}.png".format(count_img), "wb")
                        ...
                        if temp==0:
                            ...
                            temp=1
                            df.to_csv("static/data/Non_Detail/AnhNong/AnhNong.csv", mode='a', index=False, header=['img_name', 'img_src', 'title', 'doc_src'])
                        else:
                            df = pd.DataFrame(new_row)
                            df.to_csv("static/data/Non_Detail/AnhNong/AnhNong.csv", mode='a', index=False, header=False)
                else:
                    continue
            else:
                continue
...
def get_articles_img_only_AN(URL, filename):

    column = []
    temp=0
    try:
        df = pd.read_csv("static/data/Non_Detail/AnhNude/AnhNude.csv")
        column = list(set(df.doc_src.to_list()))
    except:
        pass
                ...
                if imgs.find('img'):
                    ...
                    for img in imgs1:
                        ...
                        if img.get('src'):
                            ...
                            file = open("static/data/Non_Detail/AnhNude/" +
                        ...
                        if temp==0:
                            df = pd.DataFrame(new_row)
                            temp=1
                            df.to_csv("static/data/Non_Detail/AnhNude/AnhNude.csv", mode='a', index=False, header=['img_name', 'img_src', 'title', 'doc_src'])
                        else:
                            df = pd.DataFrame(new_row)
                            df.to_csv("static/data/Non_Detail/AnhNude/AnhNude.csv", mode='a', index=False, header=False)
                else:
                    continue
            else:
                continue
...
def get_articles_img_only_VNE(URL,fn,fn1):

    column = []
    temp=0
    try:
        df = pd.read_csv("static/data/Non_Detail/VnExpress/VnExpress.csv")
        column = list(set(df.doc_src.to_list()))
    except:
        pass
            ...
            if imgs.find('img'):
                ...
                for img in imgs1:
                    ...
                    if img.get('data-src'):
                        ...
                        file = open("static/data/Non_Detail/VnExpress/"+ file_name  + "_{}.png".format(count_img), "wb")
                        count_img = count_img + 1
                        file.write(response.content)
                        file.close()
                        ...
                        if temp==0:
                            df = pd.DataFrame(new_row)
                            temp=1
                            df.to_csv("static/data/Non_Detail/VnExpress/VnExpress.csv", mode='a', index=False, header=['img_name', 'img_src', 'title', 'doc_src', 'date', 'author'])
                        else:
                            df = pd.DataFrame(new_row)
                            df.to_csv("static/data/Non_Detail/VnExpress/VnExpress.csv", mode='a', index=False, header=False)
            else:
                continue
        else:
            continue
    ...
    for item in soup.find_all('li'):
        try:
            if 'data-id' in item.attrs:
                ...
                if(os.path.exists("static/data/Non_Detail/VnExpress/"+file_name)==False):
                    os.makedirs("static/data/Non_Detail/VnExpress/"+file_name)
                ...
                for item1 in soup.find_all('li'):
                    try:
                        ...
                        if(os.path.exists("static/data/Non_Detail/VnExpress/"+file_name + "/" + sub_file_name)==False):
                            os.makedirs("static/data/Non_Detail/VnExpress/"+file_name + "/" + sub_file_name)
                        if 'data-medium' in link.attrs:
                            # print('https://vnexpress.net' + link.get('href'))
                            get_articles_img_only_VNE('https://vnexpress.net' + link.get('href'), file_name, sub_file_name)
                    except:
                        pass
        except:
            pass
...
def get_articles_img_only_TN(URL,fn,fn1):
    column = []
    temp=0
    try:
        df = pd.read_csv("static/data/Non_Detail/ThanhNien/ThanhNien.csv")
        column = list(set(df.doc_src.to_list()))
    except:
        pass
    # print(column)
                ...
                if imgs.find('img'):
                    ...
                    for img in imgs1:
                        ...
                        if img.get('data-original'):
                            ...
                            file = open("static/data/Non_Detail/ThanhNien/"+ file_name  + "_{}.png".format(count_img), "wb")
                            count_img = count_img + 1
                            file.write(response.content)
                            file.close()
                            new_row={'img_name':[file_name], 'img_src':[img.get('data-src')], 'title':[title_detail], 'doc_src':[urla], 'date':[date_time], 'author':[author]}
                            ...
                            if temp==0:
                                df = pd.DataFrame(new_row)
                                temp=1
                                df.to_csv("static/data/Non_Detail/ThanhNien/ThanhNien.csv", mode='a', index=False, header=['img_name', 'img_src', 'title', 'doc_src', 'date', 'author'])
                            else:
                                df = pd.DataFrame(new_row)
                                df.to_csv("static/data/Non_Detail/ThanhNien/ThanhNien.csv", mode='a', index=False, header=False)
                else:
                    continue
            else:
                continue
...
def crawl_img_only_TN():
    ...
    for item in soup.find_all('li'):
        try:
                ...
                link = item.find('a')
                file_name = link.get('title')
                # if(os.path.exists("static/data/Non_Detail/ThanhNien/"+file_name)==False):
                #     os.makedirs("static/data/Non_Detail/ThanhNien/"+file_name)
                reqs = requests.get(URL+link.get('href'))
                content = reqs.text
                ...
                for item1 in soup.findAll('a', {'class':'swiper-slide'}):
                    try:
                        link = item1
                        sub_file_name = link.get('title')
                        # if(os.path.exists("static/data/Non_Detail/ThanhNien/"+file_name + "/" + sub_file_name)==False):
                        #     os.makedirs("static/data/Non_Detail/ThanhNien/"+file_name + "/" + sub_file_name)
                        # if 'data-medium' in link.attrs:
                            # print('https://vnexpress.net' + link.get('href'))
                        get_articles_img_only_TN('https://thanhnien.vn' + link.get('href'), file_name, sub_file_name)
                    except:
                        pass
        except:
            pass
...
def crawl_img_only_AN():
    ...
    for item in soup.find_all('li'):
        try:
            ...
            # if(os.path.exists("static/data/Non_Detail/AnhNude/" + file_name)==False):
            #     os.makedirs("static/data/Non_Detail/AnhNude/" + file_name)
            
            if link.attrs['href'] != "/":
                get_articles_img_only_AN(link.attrs['href'], file_name)
            else:
                get_articles_img_only_AN(URL, file_name)
        except:
            pass



def crawl_img_only_XAN():
    ...
    for item in soup.find_all('li'):
        try:
            ...
            if link.attrs['href'] != "/":
                get_articles_img_only_XAN(link.attrs['href'], file_name)
            else:
                get_articles_img_only_XAN(URL)
        except:
            pass



def get_articles_img_only_NSVNE(URL,fn,fn1):

    column = []
    temp=0
    try:
        df = pd.read_csv("static/data/Non_Detail/SaoStar/SaoStar.csv")
        column = list(set(df.doc_src.to_list()))
    except:
        pass
    if(len(column)>0):
                ...
                for img in imgs1:
                    ...
                    if img.get('data-src'):
                        ...
                        if img.get('href') == '': 
                            break
                        response = requests.get(img.get('data-src'))
                        file = open('static/data/Non_Detail/SaoStar/' + file_name  + "_{}.png".format(count_img), "wb")
                        count_img = count_img + 1
                        file.write(response.content)
                        file.close()
                        ...
                        if temp==0:
                            df = pd.DataFrame(new_row)
                            temp=1
                            df.to_csv("static/data/Non_Detail/SaoStar/SaoStar.csv", mode='a', index=False, header=['img_name', 'img_src', 'title', 'doc_src', 'date', 'author'])
                        else:
                            df = pd.DataFrame(new_row)
                            df.to_csv("static/data/Non_Detail/SaoStar/SaoStar.csv", mode='a', index=False, header=False)
            else:
                continue
        else:
            continue
...
def crawl_img_only_NSVNE():
    ...
    for item in soup.find_all('li', {'class':''}):
        try:
            link = item.find('a')
            file_name = link.get('title')
            # if(os.path.exists("static/data/Non_Detail/SaoStar/"+file_name)==False):
            #     os.makedirs("static/data/Non_Detail/SaoStar/"+file_name)
            ...
            if BS(content, 'html.parser').find('div',{'class':'head-folder'}):
                ...
                for sub_link in sub_a:
                    try:
                        ...
                        # if(os.path.exists("static/data/Non_Detail/SaoStar/"+file_name + "/" + sub_file_name)==False):
                        #     os.makedirs("static/data/Non_Detail/SaoStar/"+file_name + "/" + sub_file_name)
                        get_articles_img_only_NSVNE(a, file_name, sub_file_name)
                    except:
                        pass
        except:
            pass

# python main/crawl_tongquat.py
